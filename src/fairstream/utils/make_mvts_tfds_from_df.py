...
def make_mvts_tfds_from_df(mvts_df, input_vars, output_vars, input_time_len, output_time_len, time_resolution, time_lag, batch_size):
    ...
    output_width = output_time_len//time_resolution
    shift = time_lag + output_width
    stride = input_width + shift
    w = WindowGenerator(df, input_width=input_width, output_width=output_width,
                        shift=shift, stride=stride, input_columns=input_vars, output_columns=output_vars)
