...
def viz_df_hist(df, fig_title=None, ncol=3):
    ...
    fig, axs = plt.subplots(fig_nrow, fig_ncol, figsize=(
        5*(fig_ncol+1), 5*fig_nrow))  # width, height
    axs = axs.flatten()
...
def viz_episode_ts(df, fig_title=None):
    ...
    if ("episode_relative_time" in df.columns.tolist()) and ("y" in df.columns.tolist()) and ('episode_order' in df.columns.tolist()):
        ...
        var_list = df.columns[~df.columns.isin(
            ['__uid', 'time', '__time_bin', 'y', 'episode_relative_time', 'episode_order'])]

        for var in var_list:
            fig, axs = plt.subplots(1, 3, figsize=(
                5*(3+1), 5*1), sharey='row')  # width, height
            axs = axs.flatten()
            try:
                sns.lineplot(x='episode_relative_time',
                             y=str(var), data=df, ax=axs[0])
                axs[0].set_ylabel(str(var))
                axs[0].set_xlabel('episode (all) relative time')
            except:
                axs[0].set_xlabel("Error plot "+str(var) +
                                  " over episode relative time")
                pass

            try:
                sns.lineplot(x='episode_relative_time', y=str(
                    var), hue='y', data=df, ax=axs[1])
            ...
            try:
                sns.lineplot(x='episode_relative_time', y=str(
                    var), hue='y', data=df[df['episode_order'] == 1], ax=axs[2])
...
def get_df_raw_from_csv(variable_dict, df_sample_info, source_key, file_key, viz=False):
   ...
   # loop through all subjects csvs of this file key
    for fullname in list(df_sample_info.loc[np.array(df_sample_info['source_key'] == source_key) & np.array(df_sample_info["file_key"] == file_key), 'fullname']):
        # only read colume names row
        colnames_df = pd.read_csv(
            str(fullname), index_col=0, nrows=0).columns.tolist()
        # intersect columns exist in variable dictionary
        usecols = list(set(colnames_df).intersection(set(colnames_dict)))
        df = pd.read_csv(str(fullname), low_memory=False,
                         usecols=usecols)  # read selected columns only
        df_raw = pd.concat([df_raw, df])

    if viz:
        fig_title = "Visualize Raw Dataframe from --- " + \
            str(source_key) + " --- " + str(file_key)
...
def fix_df_raw(variable_dict, df_raw, source_key):
    ...
    include_vars = []
    for var in variable_dict.keys():
        # skip variable if not in current dataframe
        if len(set(variable_dict[var]['src_names']).intersection(set(df_raw.columns.to_list()))) == 0:
            continue
        include_vars.append(var)  # append to list of all variables to include

        # --- fix variable name ---
        src_name = df_raw.columns[df_raw.columns.isin(
            variable_dict[var]['src_names'])][0]
        if var == '__anchor':
        ...
        if 'numeric' in variable_dict[var].keys():
            ...
            try:
                ...
                if variable_dict[var]['numeric']['cutoff']['quantile_max'] is not None:
                    upper = min(upper, np.quantile(df_raw[var], float(
                        variable_dict[var]['numeric']['cutoff']['quantile_max'])))
                if variable_dict[var]['numeric']['cutoff']['value_max'] is not None:
                    upper = min(upper, float(
                        variable_dict[var]['numeric']['cutoff']['value_max']))
                df_raw.loc[df_raw[var] > upper, [var]] = upper
                print("--- Fix upper boudary for " +
                      str(var) + " by " + str(upper))
            except:
            ...
            try:
                ...
                if variable_dict[var]['numeric']['cutoff']['quantile_min'] is not None:
                    lower = max(lower, np.quantile(df_raw[var], float(
                        variable_dict[var]['numeric']['cutoff']['quantile_min'])))
                if variable_dict[var]['numeric']['cutoff']['value_min'] is not None:
                    lower = max(lower, float(
                        variable_dict[var]['numeric']['cutoff']['value_min']))
                df_raw.loc[df_raw[var] < lower, [var]] = lower
                print("--- Fix lower boudary for " +
                      str(var) + " by " + str(lower))
            except:
        ...
        if 'factor' in variable_dict[var].keys():
            ...
            for level in variable_dict[var]['factor']['levels'].keys():
                df_raw.loc[df_raw[var].isin(variable_dict[var]['factor']['levels'][level]), [
                    var]] = level

            # fix subject-wise factor attributes
            for __uid in list(set(df_raw['__uid'])):
                # fix inconsistence
                # if factor level should be unique per subject (independent of time)
                if variable_dict[var]['unique_per_sbj']:
                    df_raw.loc[df_raw['__uid'] == __uid, [var]] = df_raw[df_raw['__uid']
                                                                         == __uid][var].value_counts().index[0]  # first most frequent level
                # fix invalid levels (levels not in dictionary)
                outdict_levels = list(set(
                    df_raw[df_raw['__uid'] == __uid][var])-set(variable_dict[var]['factor']['levels'].keys()))
                # if number of levels exceed nlevel_max_per_sbj, fix factor var's level limit (including y outcome var)
                if len(outdict_levels) != 0:
                    df_raw.loc[np.array(df_raw['__uid'] == __uid) & np.array(
                        df_raw[var].isin(outdict_levels)), [var]] = np.nan
                    print("--- Fix out-of-dictionry level/orders --- " +
                          str(outdict_levels) + "--- with NA for subject ---" + str(__uid))
...
def agg_df_fix(variable_dict, df_fix, time_resolution):
    ...
    if '__time' in df_fix.columns:
        df_fix['__time_bin'] = df_fix['__time']//time_resolution * \
            time_resolution  # bin fixed df by time resolution
    ...
    if len(fct_vars) == 0:  # aggregate dummied factor vars by taking the max
        ...
        df_fix = pd.get_dummies(df_fix, columns=fct_vars,
                                dtype=int, dummy_na=False, prefix_sep='___')
    ...
    df_agg = pd.merge(left=df_num_agg, right=df_fct_agg,
                      on=keys, copy=False)  # merge columns together
