...
class WindowGenerator():
    def __init__(self, df, input_width, output_width, shift, stride, input_columns=None, output_columns=None):
        ...
        # Store the raw data.
        # output_columns need to be one-hot encoded
        self.df = df[self.input_columns+self.output_columns]

        # Work out the label column indices.
        if output_columns is not None:
            self.output_columns_indices = {
                name: i for i, name in enumerate(self.output_columns)}
