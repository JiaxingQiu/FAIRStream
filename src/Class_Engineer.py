...
from func_make_mvts_df_from_csv_pool import *
from func_make_mvts_tfds_from_df import *
import random
...
class Engineer(Goblin):
    ...
    def make_mvts_df_from_csv_pool(self, csv_pool_dir, nsbj=None, frac=0.3, viz=False, viz_ts=False, stratify_by=None):
        ...
        self.mvts_df = make_mvts_df_from_csv_pool(csv_pool_dir, nsbj, frac, self.csv_source_dict, self.variable_dict, episode.input_time_len, episode.output_time_len, episode.time_resolution, episode.time_lag, episode.anchor_gap, stratify_by=stratify_by, viz=viz, viz_ts=viz_ts)
        output_vars = []
    ...
    def make_mvts_tfds_from_df(self, mvts_df, batch_size=32):
        ...
        episode=self.episode
        mvts_tfds = make_mvts_tfds_from_df(mvts_df, input_vars=self.input_vars, output_vars=self.output_vars, input_time_len=episode.input_time_len, output_time_len=episode.output_time_len, time_resolution=episode.time_resolution, time_lag=episode.time_lag, batch_size=batch_size)
        return mvts_tfds
    ...
    def split_df(self, mvts_df=None, train_frac=0.8, byepisode=False):
        ...
        self.valid_df = self.mvts_df[~self.mvts_df['split_id'].isin(train_list)]
    ...
    def fillna_by_train_df(self, train_df, valid_df, input_vars, output_vars, impute_input=None, impute_output=None, fill_value=-333):
        ...
        train_df_imputed, valid_df_imputed = cohort_fillna(train_df=train_df, valid_df=valid_df, vars=input_vars, method=impute_input, fill_value=fill_value)
        train_df_imputed, valid_df_imputed = cohort_fillna(train_df=train_df_imputed, valid_df=valid_df_imputed, vars=output_vars, method=impute_output, fill_value=fill_value)
    ...
    def BuildMVTS(self, csv_pool_dir, nsbj=None, frac=0.3, viz=False, viz_ts=False, stratify_by=None, train_frac=0.8, byepisode=False, batch_size=32, impute_input=None, impute_output=None, fill_value=-333, return_data=False):
        self.read_csv_source_dict()
        self.read_variable_dict()
        self.make_mvts_df_from_csv_pool(csv_pool_dir=csv_pool_dir, nsbj=nsbj, frac=frac, viz=viz, viz_ts=viz_ts, stratify_by=stratify_by)
        self.split_df(mvts_df=self.mvts_df, train_frac=train_frac, byepisode=byepisode)
        self.fillna_by_train_df(self.train_df, self.valid_df, input_vars=self.input_vars, output_vars=self.output_vars, impute_input=impute_input, impute_output=impute_output, fill_value=fill_value)
        
        self.train_tfds_imputed = self.make_mvts_tfds_from_df(self.train_df_imputed, batch_size=batch_size)
        self.valid_tfds_imputed = self.make_mvts_tfds_from_df(self.valid_df_imputed, batch_size=batch_size)
