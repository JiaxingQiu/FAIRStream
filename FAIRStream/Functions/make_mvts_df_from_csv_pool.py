...
def make_mvts_df_from_csv_pool(df_csv_fullname_ls, nsbj, frac, source_dict, variable_dict, input_time_len, output_time_len, time_resolution, time_lag, anchor_gap, topn_eps=None, stratify_by=None, itered=False, viz=False, viz_ts=False, dummy_na=False):
    
    df_sample_info, df_csv_fullname_ls_updated = make_sample_info_from_csv(df_file_dict=df_csv_fullname_ls, source_dict=source_dict, variable_dict=variable_dict, nsbj=nsbj, frac=frac, stratify_by=stratify_by)
    ...
    return df_episodes_ts, df_csv_fullname_ls_updated
